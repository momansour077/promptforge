...
Start with [the engineering reviewer guide](docs/REVIEWER_GUIDE.md). This is a
...
| Interface | React, Vite, Zustand, i18next, Tailwind | Component/hook tests, bilingual UI, RTL configuration |
| Async interactions | Frontend `uiAction.ts` | Success, rejected promise, synchronous failure, and fallback tests |
| Storage | Prisma/PostgreSQL and Redis | Schema, migrations, quotas; local API tests use in-memory doubles |
...
Tested on macOS arm64 with **Node 26.5.1 and npm 11.17.0**, September 5, 2026.
...
not a deployed browser. The test launcher handles the Node 26/Vitest 3 Web Storage
...
Compatible dependency resolutions were refreshed. Frontend npm audit returned
zero advisories at review time. Backend npm audit still reported three high
package entries on the same Prisma/deepmerge advisory chain and one low esbuild
entry; see the reviewer guide. Zero audit findings would not prove security.

**Not established:** updated hosted CI, real PostgreSQL/Redis behavior, paid
provider integration, real-browser E2E, Docker builds, or deployment readiness.
